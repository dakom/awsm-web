...
impl PartialWebGl2Uniforms for WebGl2RenderingContext {
    ...
    fn awsm_get_uniform_indices(&self, program: &WebGlProgram, uniform_names: &[&str]) -> Option<Vec<u32>> {
        ...
        self.get_uniform_indices(program, &arr)
            .and_then(|arr| {
                ...
                for i in 0..len {
                    let value = arr.get(i.try_into().unwrap());
    ...
    fn awsm_uniform1uiv_with_u32_array(&self, loc: &WebGlUniformLocation, data: &[u32]) {
        self.uniform1uiv_with_u32_array(Some(loc), data)
    }
    fn awsm_uniform2uiv_with_u32_array(&self, loc: &WebGlUniformLocation, data: &[u32]) {
        self.uniform2uiv_with_u32_array(Some(loc), data)
    }
    fn awsm_uniform3uiv_with_u32_array(&self, loc: &WebGlUniformLocation, data: &[u32]) {
        self.uniform3uiv_with_u32_array(Some(loc), data)
    }
    fn awsm_uniform4uiv_with_u32_array(&self, loc: &WebGlUniformLocation, data: &[u32]) {
        self.uniform4uiv_with_u32_array(Some(loc), data)
    }
}
/*

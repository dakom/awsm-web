...
pub fn start_router(window: web_sys::Window, document: web_sys::Document) -> Result<(), JsValue> {
    ...
    if let Some(menu) = menu::MENU_LOOKUP.get(pathname) {
        ...
        let href = format!(
            "https://github.com/dakom/awsm-web/tree/master/demo/rust/src/scenes/{}",
            menu.source
        );
    ...
    match pathname {
        ...
        "loaders-json" => loaders::json::start(window, document, body),
...
fn get_root(input: &str) -> &str {
    ...
    let stripped = strip_matched("awsm-web/")
        .or(Some(input))
        .unwrap();
